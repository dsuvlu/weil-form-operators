...
def _(mo):
    mo.md(r"""
    # 07. The complete Fourier matrix

    Compare the interpolation matrix, including its diagonal, with direct integration of the localized form.
...
def _(mo):
    controls = mo.ui.dictionary({
        'x': mo.ui.number(start=1.25, stop=20.0, step=0.25, value=13.0, debounce=True, label='x'),
        'N': mo.ui.number(start=1, stop=6, step=1, value=4, debounce=True, label='N'),
        'gamma_terms': mo.ui.number(start=1000, stop=10000, step=1000, value=6000, debounce=True, label='gamma_terms'),
        'y_steps': mo.ui.number(start=1000, stop=10000, step=1000, value=7000, debounce=True, label='y_steps'),
...
def _(PrecisionChoice, settings):
    ...
    x = settings["x"]
    N = int(settings["N"])
    gamma_terms = int(settings["gamma_terms"])
    y_steps = int(settings["y_steps"])
    return N, gamma_terms, precision, x, y_steps
...
def _(mo):
    ...
    Higher precision changes the formula route; the independent quadrature stays float64.
    """)
    return


@app.cell
def _(N, build_weil_matrix, gamma_terms, np, precision, x):
    H_float, indices, _ = build_weil_matrix(x, N, gamma_terms=gamma_terms)
...
def _(mo):
    ...
    The direct routine integrates each matrix entry independently. Its error depends on the quadrature grid.
    """)
    return


@app.cell
def _(N, direct_weil_matrix, x, y_steps):
    H_direct = direct_weil_matrix(x, N, y_steps=y_steps)
    return (H_direct,)
...
def _(mo):
    ...
    Both arrays and their difference are available to downstream cells.
    """)
    return


@app.cell
def _(H_direct, H_formula, np, precision):
...
@app.cell
def _(H_direct, H_formula, difference, indices, mo):
    mo.ui.table([{ "j": int(indices[i]), "formula diagonal": float(H_formula[i,i]), "direct diagonal": float(H_direct[i,i]), "difference": float(difference[i,i])} for i in range(len(indices))])
    return
...
@app.cell
def _(H_formula, indices, plt):
    __figure = plt.figure(figsize=(6.3, 5.5))
...
@app.cell
def _(H_direct, indices, plt):
    __figure_1 = plt.figure(figsize=(6.3, 5.5))
...
@app.cell
def _(difference, indices, plt):
    __figure_2 = plt.figure(figsize=(6.3, 5.5))
...
def _(mo):
    ...
    - Change y_steps without changing the formula route.
    - Inspect the zero-mode diagonal and neighboring entries separately.
    - Move x across a prime-power threshold and compare both routes.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### `fourier_pair_correlation`

    Reference: `common.py:637`.
    """)
    return
...
def _(math):
    ...
    return (fourier_pair_correlation,)
...
def _(mo):
    ...
    Reference: `common.py:656`.
    """)
    return


@app.cell
def _(fourier_pair_correlation, math, np, von_mangoldt_table):
    def direct_weil_entry(x: float, j: int, k: int, y_steps: int=6000) -> float:
        ...
        for m in range(y_steps):
            ...
            archimedean = archimedean + (rho * q - 2.0 * delta / denominator)
        polar = polar * width
        ...
        for n in range(2, max_n + 1):
            if n < x and mangoldt[n] != 0.0:
                prime = prime + mangoldt[n] / math.sqrt(n) * fourier_pair_correlation(j, k, math.log(n), L)
        return polar - c_L * delta - archimedean - prime

    return (direct_weil_entry,)
...
def _(mo):
    ...
    Reference: `common.py:699`.
    """)
    return


@app.cell
def _(direct_weil_entry, np):
...
def _(mo):
    ...
    Reference: `high_precision.py:333`.
    """)
    return
